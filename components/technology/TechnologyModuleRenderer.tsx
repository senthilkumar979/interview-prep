...
import { TechnologyQuestionItem } from './TechnologyQuestionItem'
...
export const TechnologyModuleRenderer = ({
  module,
  technologySlug,
}: TechnologyModuleRendererProps) => {
  if (module === 'Interview Questions') {
    return <TechnologyQuestionItem technologySlug={technologySlug} />
  }
  if (module === 'Coding Exercises')
    return <KataModuleSection items={kataItems as KataItem[]} />
  if (module === 'Debugging')
    return <DebugModuleSection items={debugItems as DebugItem[]} />
  if (module === 'Fill in blanks')
    return <FillModuleSection items={fillItems as FillItem[]} />
  if (module === 'MCQ')
    return <McqModuleSection items={mcqItems as McqItem[]} />
  if (module === 'Rapid Fire')
    return <RapidFireModuleSection items={rapidFireItems as RapidFireItem[]} />
  if (module === 'Output prediction')
    return (
      <OutputPredictionModuleSection
        items={outputPredictionItems as OutputPredictionItem[]}
      />
    )
  return (
    <SystemDesignModuleSection
      items={systemDesignItems as SystemDesignItem[]}
    />
  )
}
